...
use crate::{
    config,
    lux_ambiguity, lux_spec,
    lux_ticket::{self, TicketStore},
    project, session,
};
...
pub fn create_lux_mcp_server() -> McpServer {
    ...
    for tool in [
        ...
        (
            "lux_init",
            "Initialize or update a .lux workspace for a Unity project",
            lux_init_handler,
        ),
        (
            "lux_status",
            "Report current Lux server, project, session, and .lux workspace status",
            lux_status_handler,
        ),
        (
            "lux_goals",
            "Report current Lux goals, roadmap items, questions, and active work",
            lux_goals_handler,
        ),
        (
            "lux_project_info",
            "Report Unity project detection and Lux spec project information",
            lux_project_info_handler,
        ),
        (
        ...
        server.register_tool(McpTool {
            ...
            input_schema: json!({
                ...
                "properties": {
                    ...
                    "project_path": { "type": "string", "description": "Unity project root path" },
                    "force": { "type": "boolean", "description": "Force re-initialization", "default": false },
                    "interactive": { "type": "boolean", "description": "Enable interactive questions (uses AI for answers)", "default": false }
                },
                "required": []
            }),
            handler: tool.2,
...
fn lux_init_handler(arguments: &Value) -> anyhow::Result<Vec<McpContent>> {
    ...
    let lux_path = if force {
        lux_spec::lux_reinit(&project)?
    } else {
        lux_spec::lux_init(&project)?
    };

    let detection = project::detect_unity_project(&project)?;
    let mut spec = lux_spec::lux_load(&project)?;
    if let Some(ref detected_project) = detection {
        lux_spec::apply_detection_to_spec(&mut spec, detected_project);
    }
    spec.validate()
        .map_err(|error| anyhow!("Validation error: {error}"))?;
    lux_spec::lux_save(&project, &spec)?;
    ...
    text_content(json!({
        ...
        "detected_unity": detection.is_some(),
        "summary": {
            "project_name": spec.project_name,
            "status": spec_status_label(&spec.status),
            "ambiguity_score": ambiguity.overall_score,
            "defined_domains": defined_domain_names(&spec),
        }
    }))
}

fn spec_status_label(status: &lux_spec::SpecStatus) -> &'static str {
    match status {
        lux_spec::SpecStatus::Draft => "Draft",
        lux_spec::SpecStatus::Active => "Active",
        lux_spec::SpecStatus::Deprecated => "Deprecated",
    }
}

fn defined_domain_names(spec: &lux_spec::SpecProject) -> Vec<String> {
    let mut names = [
    ...
    .into_iter()
    .filter_map(|(name, domain)| domain.filter(|value| value.defined).map(|_| name.to_string()))
    .collect::<Vec<_>>();
    ...
    let mut custom = spec
        ...
        .custom
        .iter()
        .filter(|(_, domain)| domain.defined)
...
fn lux_status_handler(arguments: &Value) -> anyhow::Result<Vec<McpContent>> {
    let project = optional_project_path(arguments)?;
    let config = config::load().unwrap_or_default();
    let server = lux_server_status(&config);
    ...
    let Some(project) = project else {
        return text_content(json!({
            ...
            "project": Value::Null,
            "lux": Value::Null,
            "sessions": { "current": Value::Null, "tools": [] },
        }));
    };

    text_content(json!({
        "server": server,
        "project": project_status_summary(&project)?,
        "lux": lux_directory_overview(&project)?,
        "sessions": active_session_summary(&project)?,
    }))
}

fn lux_goals_handler(arguments: &Value) -> anyhow::Result<Vec<McpContent>> {
    let project = project_path(arguments)?;
    let spec = lux_spec::lux_load(&project).ok();
    let tickets = lux_ticket::FileTicketStore::new(&project)
    ...
    let active_tickets = tickets
        ...
        .filter(|ticket| {
            matches!(
                ticket.status,
                lux_ticket::TicketStatus::InProgress | lux_ticket::TicketStatus::Blocked
            )
        })
        .map(|ticket| {
            json!({
                ...
                "spec_ref": ticket.spec_ref,
                "updated_at": ticket.updated_at,
            })
        })
        .collect::<Vec<_>>();
    let ticket_counts = ticket_status_counts(&tickets);
    ...
    let Some(spec) = spec else {
        return text_content(json!({
            ...
            "roadmap": [],
            "active_work": {
                "current_session": current_session_value(&project),
    ...
    text_content(json!({
        ...
        "goals": spec_goal_summary(&spec),
        "open_questions": spec.dialectic.questions.iter().filter(|question| question.answer.is_none()).map(|question| json!({
            "id": question.id,
        ...
        "roadmap": spec.roadmap.tickets.iter().map(|ticket| json!({
            "id": ticket.id,
            "title": ticket.title,
            "domain": ticket.domain,
            "requirement_refs": ticket.requirement_refs,
...
fn lux_project_info_handler(arguments: &Value) -> anyhow::Result<Vec<McpContent>> {
    let project = project_path(arguments)?;
    let detection = project::detect_unity_project(&project)?;
    let spec = lux_spec::lux_load(&project).ok();
    ...
    text_content(json!({
        "project_path": project,
        "unity": detection.as_ref().map(|detection| json!({
            "detected": true,
            "root": detection.root,
            "project_name": detection.project_name,
            "editor_version": detection.editor_version,
            "render_pipeline": detection.render_pipeline,
            "scripting_backend": detection.scripting_backend,
            "target_platforms": detection.target_platforms,
            "test_framework_detected": detection.test_framework_detected,
            "packages": detection.packages.iter().map(|package| json!({
                "name": package.name,
                "version": package.version,
            })).collect::<Vec<_>>(),
        })).unwrap_or_else(|| json!({ "detected": false })),
        "spec": spec.as_ref().map(spec_project_info).unwrap_or_else(|| json!({ "present": false })),
        "lux": lux_directory_overview(&project)?,
    }))
}

fn optional_project_path(arguments: &Value) -> anyhow::Result<Option<PathBuf>> {
    if let Some(path) = arguments.get("projectPath").and_then(Value::as_str) {
    ...
    if let Some(info) = project::detect_from_cwd()? {
        return Ok(Some(info.root));
    }
    Ok(None)
}

fn lux_server_status(config: &config::LuxConfig) -> Value {
    json!({
        "running": is_tcp_port_open(&config.server.host, config.server.port),
...
fn project_status_summary(project: &Path) -> anyhow::Result<Value> {
    let detection = project::detect_from_path(project)?;
    Ok(detection
        .map(|info| {
            json!({
                "path": info.root,
        ...
        .unwrap_or_else(|| json!({
            "path": project,
            "name": project.file_name().and_then(|name| name.to_str()).unwrap_or("unknown"),
            "unity_version": Value::Null,
        })))
}

fn lux_directory_overview(project: &Path) -> anyhow::Result<Value> {
    let lux_root = project.join(".lux");
    if !lux_root.exists() {
        return Ok(json!({
            "path": lux_root,
            "exists": false,
    ...
    for entry in fs::read_dir(&lux_root)
        .with_context(|| format!("failed to read {}", lux_root.display()))?
    {
        let entry = entry?;
        let path = entry.path();
        ...
        entries.push(json!({
            ...
            "modified_unix": metadata.modified().ok().and_then(|time| time.duration_since(UNIX_EPOCH).ok()).map(|duration| duration.as_secs()),
            "path": path,
        }));
    }
    ...
    Ok(json!({
        ...
        "exists": true,
        "entries": entries,
        "spec_present": lux_root.join("spec.json").is_file(),
        "ai_log_present": lux_root.join("ai-action-log.jsonl").is_file(),
        "sessions_present": lux_root.join("sessions").is_dir(),
        "tickets_present": lux_root.join("tickets").is_dir(),
        "skills_present": lux_root.join("skills").is_dir(),
    }))
}

fn active_session_summary(project: &Path) -> anyhow::Result<Value> {
    let tool_sessions = session::read_tool_sessions(project)
        .unwrap_or_default()
        .into_iter()
        .filter(|record| record.status != "closed" && record.status != "stopped")
        .map(|record| {
            json!({
                "id": record.id,
                ...
                "command_count": record.command_history.len(),
    ...
    Ok(json!({
        "current": current_session_value(project),
        "tools": tool_sessions,
    }))
}

fn current_session_value(project: &Path) -> Value {
    session::current_session_id(project)
        .map(Value::String)
        .unwrap_or(Value::Null)
}

fn spec_goal_summary(spec: &lux_spec::SpecProject) -> Vec<Value> {
    let mut goals = Vec::new();
    for (name, domain) in spec_domain_entries(spec) {
        if !domain.defined && domain.goals.is_empty() && domain.requirements.is_empty() {
            continue;
        }
        goals.push(json!({
            "domain": name,
            "defined": domain.defined,
            "status": format!("{:?}", domain.status),
            "goals": domain.goals,
            "non_goals": domain.non_goals,
            "requirements": domain.requirements.iter().map(|requirement| json!({
                "id": requirement.id,
                "text": requirement.text,
                "priority": format!("{:?}", requirement.priority),
                "status": format!("{:?}", requirement.status),
...
fn spec_domain_entries(spec: &lux_spec::SpecProject) -> Vec<(&str, &lux_spec::DomainSpec)> {
...
fn ticket_status_counts(tickets: &[lux_ticket::Ticket]) -> Value {
    let mut counts = serde_json::Map::new();
    for ticket in tickets {
        let key = format!("{:?}", ticket.status);
...
fn spec_project_info(spec: &lux_spec::SpecProject) -> Value {
    json!({
        "present": true,
...
mod tests {
    ...
    #[test]
    fn lux_server_registers_twenty_six_tools_and_three_resources() {
        let response = response_json(
    ...
    #[test]
    fn create_lux_mcp_server_registers_twenty_six_tools() {
        let response = response_json(
            &create_lux_mcp_server(),
