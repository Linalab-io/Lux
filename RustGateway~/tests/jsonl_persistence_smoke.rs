...
fn empty_env_var_falls_back_to_default_path() {
    ...
    assert_eq!(
        resolved,
        PathBuf::from("/project/.lux/ai-action-log.jsonl")
    );
...
fn append_work_step_writes_multiple_entries_in_order() {
    ...
    assert_eq!(lines.len(), 4, "Expected 4 appended lines, got {}", lines.len());
...
fn legacy_log_migrated_from_user_settings_to_lux() {
    ...
    // Rewrite properly
    fs::write(&legacy_path, r#"{"actor":"legacy-agent"}"#).unwrap();
    fs::OpenOptions::new().append(true).open(&legacy_path).unwrap().write_all(b"\n").ok();
    ...
    assert!(!legacy_path.exists(), "Legacy file must be removed after migration");
...
fn legacy_migration_skipped_when_new_path_already_exists() {
    ...
    assert!(legacy_path.exists(), "Legacy must remain when new path already exists");
