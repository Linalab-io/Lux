...
fn event_source_rejects_unknown_variant_gracefully() {
    ...
    assert!(result.is_err(), "unknown EventSource variant should fail to deserialize");
}
...
fn event_category_all_variants_serialize_correctly() {
    ...
    for (variant, name) in expected {
        ...
        assert_eq!(v, json!(name), "EventCategory::{variant:?} should serialize as {name}");
    }
}
...
fn redaction_metadata_serializes_camelcase_field_names() {
    ...
    assert!(value.get("redactedFields").is_some(), "expected camelCase 'redactedFields'");
    assert!(value.get("redactionClasses").is_some(), "expected camelCase 'redactionClasses'");
    assert!(value.get("timestamp").is_some(), "expected 'timestamp' field");
    // Must NOT contain snake_case
    let s = serde_json::to_string(&value).unwrap();
    assert!(!s.contains("redacted_fields"), "snake_case leaked into JSON");
    assert!(!s.contains("redaction_classes"), "snake_case leaked into JSON");
}
...
fn retention_metadata_serializes_camelcase_field_names() {
    ...
    let value = serde_json::to_value(&meta).unwrap();
    assert!(value.get("maxAgeDays").is_some(), "expected camelCase 'maxAgeDays'");
    assert!(value.get("maxLines").is_some(), "expected camelCase 'maxLines'");
    assert!(value.get("policy").is_some(), "expected 'policy' field");
    assert!(value.get("createdAt").is_some(), "expected camelCase 'createdAt'");
    assert!(value.get("expiresAt").is_some(), "expected camelCase 'expiresAt'");
}
...
fn lux_event_full_populated_roundtrips() {
    ...
    assert_eq!(deserialized.event_id, "a1b2c3d4-e5f6-7890-abcd-ef1234567890");
...
fn all_source_category_combinations_serialize() {
    ...
    for (i, source) in sources.iter().enumerate() {
        for (j, category) in categories.iter().enumerate() {
            ...
            let deserialized: EventEnvelope = serde_json::from_str(&serialized)
                .unwrap_or_else(|e| panic!("failed to deserialize {:?}x{:?}: {e}", source, category));
            assert_eq!(
